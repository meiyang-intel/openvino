...
NamedOutputs elementwise_ops(const NodeContext& node) {
    ...
    if (axis_value == -1)
        ...
        // condition of when x_rank == y_rank
        auto rank_is_equal = std::make_shared<ngraph::opset6::Equal>(x_rank, y_rank);
        auto revised_axis = std::make_shared<ngraph::opset6::Select>(rank_is_equal, const_0, axis);

        // generate broadcast node of y
        auto revised_axis_scalar = std::make_shared<ngraph::opset6::Squeeze>(revised_axis);
        auto range0 = std::make_shared<ngraph::opset6::Range>(const_0, revised_axis_scalar, const_1, ngraph::element::i64);
        auto y_rank_add_axis = std::make_shared<ngraph::opset6::Add>(y_rank, revised_axis);
        auto y_rank_add_axis_scalar = std::make_shared<ngraph::opset6::Squeeze>(y_rank_add_axis);
        auto x_rank_scalar = std::make_shared<ngraph::opset6::Squeeze>(x_rank);
        auto range1 = std::make_shared<ngraph::opset6::Range>(y_rank_add_axis_scalar, x_rank_scalar, const_1, ngraph::element::i64);
        auto indices = std::make_shared<ngraph::opset6::Concat>(ngraph::NodeVector{range0, range1}, 0);
        auto y_node = std::make_shared<ngraph::opset6::Unsqueeze>(y, indices);
