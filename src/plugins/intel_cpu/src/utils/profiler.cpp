...
struct PMUMonitor : DumbVM::VM {
    ...
    PMUMonitor(const char* config_file_path) {
        ...
        if (input_file.is_open()) {
            ...
            std::cerr << "==== Profile config file not found:" << config_file_path << std::endl;
        }
        initialize();
    }
    void initialize() {
...
ProfileData::ProfileData(const std::string& name) : name(name) {
    auto & c = profilerManagerInstance.addCounter();
    c.end = start = __rdtsc();
}
...
ProfilerManager::ProfilerManager() {
    ...
    if (!str_enable)
        str_enable = "0";
    const char* dump_file = std::getenv("OV_CPU_PROFILE_FILE");
    if (dump_file) {
...
static std::ostringstream dump_text;
...
void ProfilerManager::finalize() {
    ...
    if (all_data.size()) {
        ...
        chromeTrace ct(dump_text, fake_tid);
        for (auto& d : all_data) {
